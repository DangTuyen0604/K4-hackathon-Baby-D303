...
* **Case 15 (SQL Injection):** `DROP TABLE qa_cache;--` $\rightarrow$ *Parameterized Query xử lý an toàn*.
* **Case 16 (Standard Parsing):** Thông báo 500 từ $\rightarrow$ *Tóm tắt ngắn 2 mục Lịch học & Deadline*.
* **Case 17 (Multi-deadlines):** 3 deadline khác nhau $\rightarrow$ *Trích xuất đầy đủ cả 3*.
* **Case 18 (No Deadline):** Thông báo chúc mừng sinh nhật $\rightarrow$ *Báo Không có Action Item*.
* **Case 19 (Empty Channel):** Không có tin nhắn mới $\rightarrow$ *Báo Không có thông báo mới*.
* **Case 20 (Mixed Chat):** Tán gẫu xen kẽ thông báo $\rightarrow$ *Lọc bỏ tin tán gẫu*.
* **Case 21 (Links & Files):** Có link Google Form $\rightarrow$ *Trích xuất chính xác link*.
